...
RUN apt-get update \
    ...
    && curl --fail --location --silent --show-error --output cmake-3.28.3-linux-x86_64.tar.gz \
...
# Ignore working directory in cache keys so warmup cache (built in /workspace/) is reused
# by CI builds (which run in different checkout paths like /__w/presto/presto/)
...
RUN curl --fail --location --silent --show-error --output OpenJDK8U-jdk_x64_linux_hotspot_8u442b06.tar.gz \
